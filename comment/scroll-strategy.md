...
| 패턴 | 설명 | 데모 |
|------|------|------|
| **Y-Snap** | `scroll-snap-type: y proximity` + 섹션별 `scroll-snap-align: start` | [01-y-proximity](01-y-proximity.html) |
| **가로 전환** | sticky 컨테이너 안에서 세로 스크롤 → 가로 슬라이드 (GSAP ScrollTrigger) | [02-horizontal-scroll](02-horizontal-scroll.html) |
| **Sticky Sidebar** | 왼쪽 네비 고정, 오른쪽 콘텐츠 자유 스크롤, 현재 위치 하이라이트 | [03-sticky-sidebar](03-sticky-sidebar.html) |
| **영상 확대** | 스크롤 진행에 따라 영상이 60% → 100vw 풀스크린으로 확대 | [04-video-expand](04-video-expand.html) |
...
| # | 섹션 | 스냅 | 추가 인터랙션 |
|---|------|:---:|------|
| 1 | 메인 배너 (비디오, "경계를 넘어, 내일을 열다") | **start** | — |
| 2 | 혁신전략 (4카드: Global Alliance, Field Campus 등) | **start** | — |
| 3 | Field 캠퍼스 (6탭 카드) | **start** | — |
| 4 | 핵심 성과 (수강생 2,363 / 매출 918억 / 유학생 3,898) | **start** | CountUp 애니메이션 |
| 5 | 글로컬 뉴스 (4카드) | **start** | — |
| 6 | FAQ (4항목 아코디언) | **start** | — |
| 7 | 푸터 | 제외 | — |
...
**전략: 상단 Y-Snap + 8대 추진과제 가로 전환**

| # | 섹션 | 패턴 | 설명 |
|---|------|------|------|
| 1 | 히어로 배너 (SCROLL DOWN) | **Y-Snap** | 첫 인상 |
| 2 | 글로컬대학30 프로젝트 (비전/목표/전략 I·II) | **Y-Snap** | 정보 요약형, viewport에 맞음 |
| 3 | OUR VISION (3개 혁신방향 × 5 세부과제) | **Y-Snap** | 카드형 구조 |
...
**전략: 가장 복합적 — Y-Snap + 가로 전환 + Sticky Sidebar + 영상 확대**

| # | 섹션 | 패턴 | 설명 |
|---|------|------|------|
| 1 | TECHBRIDGE 메인 (비교 다이어그램) | **Y-Snap** | 첫 인상, 마우스 아이콘 |
| 2 | 3 BUSINESS MODELS (3카드: 프랜차이징/마켓부스팅/스노우볼링) | **Y-Snap** | viewport에 맞는 카드형 |
| 3 | **3 CORE STRATEGIES** | **가로 전환** | 전략 01→02→03 |
| 4 | **ACHIEVEMENT** (조직/대외협력/기술사업화) | **Sticky Sidebar** | 좌측 네비 고정 + 우측 자유 스크롤 |
| 5 | **VISION VIDEO** ("통합산단 2.0") | **영상 확대** | 60%→풀스크린 |
| 6 | PRESS RELEASE (6개 뉴스 카드) | **Y-Snap** | 뉴스 그리드 |
| 7 | 푸터 | 제외 | — |
...
| # | 섹션 | 스냅 | 이유 |
|---|------|:---:|------|
| 1 | 인트로 배너 ("無경계", 배경 이미지) | **start** | 깔끔한 첫 진입 |
...
| # | 섹션 | 스냅 | 이유 |
|---|------|:---:|------|
| 1 | 인트로 ("Field 연합전공이란?") | **start** | 정의, viewport에 맞음 |
| 2 | 운영현황 (6개 탭, MajorTabs) | 제외 | 탭 전환 시 높이 가변 → 스냅과 충돌 |
| 3 | 신청방법 (3열 + 3단계 절차) | **start** | 안내형, viewport에 맞음 |
| 4 | 이수체계 (로드맵 + 아코디언 FAQ) | 제외 | 아코디언 확장 시 높이 가변 |
| 5 | 혜택 (장학금 표 + 4카드) | **start** | 마무리, viewport에 맞음 |
| 6 | 푸터 | 제외 | — |
...
| 페이지 | Y-Snap | 가로 전환 | Sticky Sidebar | 영상 확대 | 2단 스냅 | 자유 스크롤 |
|--------|:---:|:---:|:---:|:---:|:---:|:---:|
| `/` 메인 | 전체 | — | — | — | — | 푸터 |
